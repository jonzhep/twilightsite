...
import { DiscardMaterial } from "./DiscardMaterial.ts";
...
function getRendertarget(
  ...
  const _height =
    typeof height === "number" ? height : window.innerHeight * dpr;
  const _settings =
...
interface MeshTransmissionMaterialParameters
...
export class MeshTransmissionMaterial extends THREE.MeshPhysicalMaterial {
  ...
  constructor(
    ...
    this.onBeforeCompile = (
      shader: THREE.Shader & { defines: { [key: string]: string } }
    ) => {
      shader.uniforms = {
        ...shader.uniforms,
        ...this.uniforms,
  ...
  update(
    state: MeshTransmissionMaterialUpdateState,
    target: THREE.Mesh<THREE.BufferGeometry, this>
  ) {
